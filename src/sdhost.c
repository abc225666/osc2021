...
int sd_mount() {
    ...
    memcpy(&mbr_part, buf+446, sizeof(struct mbr_partition));

    if(mbr_part.partition_type == 0x0b) { // FAT32 CHS
        readblock(mbr_part.starting_sector, buf);
        char mountpoint[8] = "/sd";
        vfs_mkdir(mountpoint);
        vfs_mount("sdcard", mountpoint, "fat32");


        struct fat32_boot_sector *boot_sector = (struct fat32_boot_sector *)buf;
        fat32_metadata.data_region_blk_idx = mbr_part.starting_sector +
                                             boot_sector->n_sectors_per_fat_32 * boot_sector->n_file_alloc_tabs +
                                             boot_sector->n_reserved_sectors;
        fat32_metadata.fat_region_blk_idx = mbr_part.starting_sector + boot_sector->n_reserved_sectors;
